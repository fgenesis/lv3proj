...
// WARNING: The code in this file SUCKS. Badly.
...
{
    PackDef()
        : level(LVPACOMP_INHERIT), algo(LVPAPACK_INHERIT), encrypt(LVPAENCR_INHERIT), solid(false), exec(NULL)
    {}
...
static void usage(void)
{
    printf("lvpak MODE archive [-flags files/dirs ..]\n"
         ...
         //"  -D DIR - same as -d, but recursive\n"
           ...
           "     A - can be none, lzo, lzma, or i (inherit)\n"
           ...
           "     b  - if -Kb is used, tread the key as hexadecimal binary string.\n"
           ...
           "\n"
           "* lvpak uses LZMA by Igor Pavlov, and LZO by Markus F.X.J. Oberhumer *\n"
           );
}
...
static bool parseCompressString(const char *str, uint8 *algo, uint8 *level)
{
    ...
    logerror("malformed compression parameters: '%s'", str);
    return false;
}
