...
struct AnimatedTile
{
    ...
    AnimatedTile(Anim *a, uint32 idx = 0, char *startwith = NULL);
    Anim *ani;
    AnimFrame *curFrame;
    ...
    void SetupDefaults(uint32 idx = 0, char *startwith = NULL);
    void Init(uint32 t); // current system clock
    static void SplitFilenameToProps(char *in, std::string *fn, uint32 *idx, std::string *str);
};
